...
def test_workspace_context_isolated_and_resolves_resources():
    ...
    first = Workspace(
        id=str(uuid4()), slug="first", name="First", state="active", created_at=now, updated_at=now
    )
    second = Workspace(
        id=str(uuid4()),
        slug="second",
    ...
    for workspace in (first, second):
        session.add(
            WorkspaceResource(
                ...
                resource_type="qdrant_chunks",
                logical_name="active",
                backend_name="cortex_chunks",
                active=True,
                created_at=now,
            )
        )
        session.add(
            WorkspaceResource(
                id=str(uuid4()),
                workspace_id=workspace.id,
                resource_type="graphrag_root",
                logical_name="active",
                path=f"workspaces/{workspace.id}/graphrag",
                active=True,
                created_at=now,
            )
        )

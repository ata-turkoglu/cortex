...
def upgrade():
    ...
    op.add_column("messages", sa.Column("status", sa.String(32), nullable=False, server_default="completed"))
    ...
    for name, type_, nullable, default in (
        ...
        op.add_column("query_runs", sa.Column(name, type_, nullable=nullable, server_default=default))


def downgrade():
    raise NotImplementedError("Downgrade is intentionally unsupported for this data-bearing migration")

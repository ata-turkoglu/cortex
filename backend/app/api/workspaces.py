...
def lookup(s, id):
    try:
        return WorkspaceContext.load(s, id).workspace
    except WorkspaceNotFoundError as exc:
        raise HTTPException(404, "workspace not found") from exc


@router.get("", response_model=list[WorkspaceRead])
def list_workspaces(s: Session = Depends(get_session)):
    return s.scalars(
...
def create(payload: WorkspaceCreate, s: Session = Depends(get_session)):
    ...
    if s.scalar(select(Workspace.id).where(Workspace.slug == payload.slug)):
    ...
    for typ, name, path in [
        ...
        ("normalized", None, f"{base}/normalized"),
    ]:
        s.add(
            WorkspaceResource(
                id=str(uuid4()),
...
def get(workspace_id: str, s: Session = Depends(get_session)):
    return lookup(s, workspace_id)

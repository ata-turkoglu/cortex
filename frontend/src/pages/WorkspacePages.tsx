...
export function WorkspacesPage() {
  const { workspaces, error, refresh } = useWorkspaces(); const [name, setName] = useState(""); const [description, setDescription] = useState(""); const [saving, setSaving] = useState(false); const [createError, setCreateError] = useState("");
  const create = async (event: FormEvent) => { event.preventDefault(); const slug = slugify(name); if (!slug) { setCreateError("Çalışma alanı adı en az bir harf veya sayı içermeli."); return; } setSaving(true); setCreateError(""); try { await apiClient.createWorkspace(name.trim(), slug, description.trim()); setName(""); setDescription(""); await refresh(); } catch (reason) { setCreateError(reason instanceof Error ? reason.message : "Çalışma alanı oluşturulamadı."); } finally { setSaving(false); } };
  return <div className="page-stack two-column"><ACard title="Çalışma alanları">{error && <p role="alert">{error}</p>}<div className="resource-list">{workspaces.map((w) => <Link key={w.id} to={`/workspace/${w.id}`}><strong>{w.name}</strong><span>{w.description || w.slug}</span></Link>)}{!workspaces.length && <AInfo title="İlk çalışma alanınızı oluşturun">Her alan, belgelerini ve indekslerini birbirinden izole tutar.</AInfo>}</div></ACard><ACard title="Yeni çalışma alanı"><form className="form-stack" onSubmit={create}><AInput value={name} onChange={(e) => setName(e.target.value)} placeholder="Örn. Osmanlı arşivi" required /><ATextarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Kısa açıklama (isteğe bağlı)" rows={3} /><AButton type="submit" label={saving ? "Oluşturuluyor…" : "Oluştur"} disabled={saving} />{createError && <p role="alert">{createError}</p>}</form></ACard></div>;
}

function slugify(value: string) {
  return value.trim().toLocaleLowerCase("tr-TR").replaceAll("ı", "i").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
}

...
import { formatCortexDateTime } from "../utils/date";
...
export function FailedJobsPage() {
  ...
  return <div className="page-stack">
    ...
    <ACard title="Kurtarılabilir işler">{runs.length ? <div className="document-table">{runs.map((run) => <div key={run.id}><div><strong>{run.job_type}</strong><span>{run.recovery_state || run.state} · {formatCortexDateTime(run.updated_at)}</span></div><div className="flex gap-2"><AButton label="Ayrıntı" text onClick={() => void showDetails(run)} /><AButton label="Yeniden dene" onClick={() => void apiClient.retryWorkflow(run.id).then(refresh)} /></div></div>)}</div> : <AInfoPanel title="Kurtarma gerektiren iş yok">Başarısız veya kesintiye uğramış süreç bulunmuyor.</AInfoPanel>}</ACard>
